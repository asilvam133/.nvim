-- Taken from https://github.com/kristijanhusak/neovim-config/blob/master/nvim/lua/partials/statusline.lua
local statusline = {
    cwd_folder = '',
    lsp_progress = '',
...
vim.api.nvim_create_autocmd('FileType', {
    ...
    callback = function()
        vim.opt_local.statusline = '%!v:lua.require("statusline").setup()'
    end,
})
...
local function lsp_diagnostics()
    ...
    if err_count > 0 then
        table.insert(
            ...
            sep(
                ' ' .. err_count,
                vim.tbl_extend('keep', { no_after = warn_count == 0 }, section_err_right),
                err_count > 0
            )
        )
    end
...
local function filetype()
    ...
    if ft_icon and ft_icon ~= '' and ft_icon_hl and ft_icon_hl ~= '' and c.statusline_bg ~= '' then
...
_G.asilva.get_workspace_name = get_workspace_name
